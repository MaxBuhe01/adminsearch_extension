...
use OxidEsales\Eshop\Application\Controller\Admin\NavigationController as NavigationController;
...
if (false){
  class_alias(NavigationController::class, mb_navigationController_extension_parent::class);
}

class mb_navigationController_extension extends mb_navigationController_extension_parent
{
  /**
   * @var null|object
   */
  ...
  public function getOxcomAdminSearchResults()
  {
    ...
    if (strlen($this->_sQueryName) > 2) {
      ...
      if ($this->getOxcomAdminSearchConfigParam("blMbAdminSearchShowModulesLinks")) {
        $aData["links"] = $this->_getOxcomAdminSearchLinks();
      }
    }
   ...
   * @return object
   */
  public function _getOxcomAdminSearchLinks($sParam = '')
  {
    $sViewName = $this->_sViewNameGenerator->getViewName("oxlinks");
    $sSql = "SELECT oxid, oxurl FROM $sViewName WHERE oxurl LIKE " . \OxidEsales\Eshop\Core\DatabaseProvider::getDb()->quote('%' . $this->_sQueryName . '%');

    return $this->_getOxcomAdminSearchData($sSql, 'admin_links');
  }
